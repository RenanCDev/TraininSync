...
import { updateAluno } from "../../../api/aluno/updateAluno";
import { CreateAluno } from "../register-student/zod";
...
export function EditStudent() {
  ...
  const {
    ...
    defaultValues: {
      ...
      email: aluno?.email ?? "",
      numero_de_celular: formatPhoneNumber(aluno?.numero_de_celular ?? ""),
      sexo: aluno?.sexo ?? "N",
      etnia: aluno?.etnia ?? "nao_informado",
      estado_civil: aluno?.estado_civil ?? "nao_informado",
      altura: aluno?.altura ?? "",
      peso: aluno?.peso ?? "",
      imc: aluno?.imc ?? "",
      bioimpedancia: aluno?.bioimpedancia ?? "",
      data_do_exame: aluno?.data_do_exame ?? DataAtual,
      hora_do_exame: aluno?.hora_do_exame ?? "",
      agua_corporal_total: aluno?.agua_corporal_total ?? "",
      proteinas: aluno?.proteinas ?? "",
      minerais: aluno?.minerais ?? "",
      gordura_corporal: aluno?.gordura_corporal ?? "",
      massa_muscular_esqueletica: aluno?.massa_muscular_esqueletica ?? "",
      taxa_metabolica_basal: aluno?.taxa_metabolica_basal ?? "",
    },
  });
  ...
  const onSubmit = async (data: AlunoFormData) => {
    ...
    try {
      setIsLoading(true);
      await updateAluno(Number(id), cleanData);
